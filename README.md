...
Для воспроизводимого baseline теперь есть отдельная команда обучения. Она использует только:

- `bibliography.title`
- `content.abstract`

Из baseline автоматически исключаются записи с `gold_label` = `partial` и `unknown`. Fulltext, теги, `notes`, `seed_query`, `raw.*` и прочие metadata leakage-поля в обучение не попадают.

### Запуск на Windows

```powershell
python -m vkr_article_dataset.cli train-baseline `
  --input data\normalized\articles.jsonl `
...
После запуска в `artifacts\baseline_run_01\` появятся:

- `dataset\baseline_dataset.jsonl` и `dataset\baseline_dataset.csv`;
- `splits\train.jsonl`, `splits\val.jsonl`, `splits\test.jsonl`, `splits\manifest.json`;
- `logreg\` с `vectorizer.joblib`, `model.joblib`, `config.json`, `metrics_*.json`, `confusion_matrix_*.json`, `predictions_*.csv`;
- `linear_svm\` с таким же набором артефактов;
- `summary.json` с общей сводкой запуска.
